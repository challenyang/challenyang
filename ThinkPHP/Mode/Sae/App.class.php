...
// +----------------------------------------------------------------------
// $Id: App.class.php 21 2011-06-27 03:19:03Z luofei614@126.com $
 ...
 * @subpackage  Core
 * @author    liu21st <liu21st@gmail.com>
 * @version   $Id: App.class.php 21 2011-06-27 03:19:03Z luofei614@126.com $
 +------------------------------------------------------------------------------
 */
class App
{//类定义开始
    ...
    static public function init()
    {
        ...
        $content=$cache->get('~'.APP_CACHE_NAME.'.php');
        ...
        }elseif($content!==false && ( !is_file(CONFIG_PATH.'config.php') || $cache->mtime>filemtime(CONFIG_PATH.'config.php') )) {
            // 直接读取编译后的项目文件
            C(eval("?>".$content));
        }else{
            // 预编译项目
     ...
     */   static private function build()
    {
        ...
        if($debug) {
            ...
            if($runtime) {
                ...
                $content .= substr($cache->get($runtimefile),5);
                $content .= $common."\nreturn ".var_export(C(),true).';';
                //file_put_contents(RUNTIME_PATH.'~allinone.php',strip_whitespace('<?php '.$content));
                $cache->set('~allinone.php',strip_whitespace('<?php '.$content));
            }else{
                $content  = "<?php ".$common."\nreturn ".var_export(C(),true).";\n?>";
               $cache->set('~'.APP_CACHE_NAME.".php",$content);
