...
{//类定义开始
    ...
    public function loadTemplate ($tmplTemplateFile='')
    {
        ...
        $tmplCacheFile = "Tpl/".md5($tmplTemplateFile).$this->config['cache_suffix'];
        $tmplContent = '';
        // 检查Cache文件是否需要更新
        $cache=Tplcache::getInstance();
        $content=$cache->get($tmplCacheFile);
        if ($content===false || !$this->checkCache($tmplTemplateFile,$cache->mtime)) {
            // 需要更新模版 读出原模板内容
            ...
            //重写Cache文件
            if( false === $cache->set($tmplCacheFile,trim($tmplContent)))
                throw_exception(L($s->errmsg()));
            $content=$cache->get($tmplCacheFile);
        }
        return $content;
    }
    ...
    protected function checkCache($tmplTemplateFile,$mtime)
    {
       ...
       if (filemtime($tmplTemplateFile) > $mtime){
            // 模板文件如果有更新则缓存需要更新
            return false;
        }elseif ($this->config['cache_time'] != -1 && time() > $mtime+$this->config['cache_time']) {
            // 缓存是否在有效期
            return false;
        }
        //缓存有效
